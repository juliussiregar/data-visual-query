    ...
    <aside className="space-y-3 xl:sticky xl:top-24 xl:max-h-[calc(100vh-7rem)] xl:overflow-y-auto">
      ...
      <div className="rounded-2xl border border-white/10 bg-slate-900/60 p-4">
        <div className="mb-2 flex items-center justify-between text-xs">
          <span className="font-medium text-white">Widget aktif</span>
          <span className="text-indigo-300">{visibleCount}/{sorted.length}</span>
        </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-indigo-500/20 bg-indigo-500/5">
        ...
        >
          <span className="text-xs font-semibold text-indigo-200">Cara pakai</span>
        ...
        {guideOpen && (
          <ol className="space-y-2 border-t border-indigo-500/15 px-4 pb-4 pt-2 text-[11px] text-slate-400">
            ...
            <li>4. Klik <strong className="text-slate-300">Selesai</strong></li>
          </ol>
        )}
      </div>
          ...
          {LAYOUT_TEMPLATES.map((t) => {
            ...
            return (
              <button
                ...
                className="group flex w-full items-center gap-3 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-3 text-left transition-all hover:border-violet-500/40 hover:bg-violet-500/10"
              >
              ...
              >
                <div className="flex items-center gap-2">
                  <label className="flex min-w-0 flex-1 cursor-pointer items-center gap-2.5">
                    <input
                      ...
                      className="h-4 w-4 shrink-0 rounded border-white/20 accent-indigo-500"
                    />
                    <div
                      className={cn(
                        "flex h-7 w-7 shrink-0 items-center justify-center rounded-lg",
                        widget.visible ? "bg-indigo-500/20" : "bg-white/5"
                      )}
                    >
                      <Icon className={cn("h-3.5 w-3.5", widget.visible ? "text-indigo-300" : "text-slate-600")} />
                    </div>
                    <span
                      className={cn(
                        "truncate text-xs",
                  ...
                  {widget.visible && (
                    <div className="flex shrink-0 overflow-hidden rounded-lg border border-white/10">
                      <button
                        ...
                        className="px-1.5 py-1 text-slate-400 hover:bg-white/10 hover:text-white disabled:opacity-25"
                      >
                      ...
                      <button
                        ...
                        className="border-l border-white/10 px-1.5 py-1 text-slate-400 hover:bg-white/10 hover:text-white disabled:opacity-25"
                      >
                ...
                {widget.visible && onScrollToWidget && (
                  <button
                    ...
                    className="mt-2 text-[10px] font-medium text-indigo-400/80 hover:text-indigo-300"
                  >
                    Lihat di preview →
                  </button>
                )}
